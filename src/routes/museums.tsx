...
function MuseumsPage() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      <Navbar />
      <PageHeader
        eyebrow="Hamkor muzeylar"
        title="O'zbekiston"
        highlight="muzeylari"
        subtitle="Toshkent, Samarqand, Buxoro, Nukus — har bir muzey tarixning alohida bo'limini hikoya qiladi."
      />
      ...
      <section className="bg-background py-20">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-6 md:grid-cols-2 lg:px-10">
          {MUSEUMS.map((m) => (
            <article
              key={m.id}
              className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card shadow-card transition-smooth hover:border-primary/50 hover:-translate-y-1"
            >
              ...
              <div className="p-7">
                ...
                <div className="mt-6 flex items-center justify-between border-t border-border/50 pt-5">
                  <div className="text-xs text-muted-foreground">
                    <span className="font-serif text-primary">{m.exhibits}</span> eksponat
                  </div>
                  <Link
                    to="/shop"
