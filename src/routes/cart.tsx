      ...
      <section className="pt-32 pb-24 lg:pt-40">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <h1 className="mt-4 font-serif text-5xl leading-[1.05] sm:text-6xl">
            Tanlangan <span className="italic text-primary">buyumlar</span>
          </h1>
          ...
          {items.length === 0 ? (
            <div className="mt-16 grid place-items-center rounded-2xl border border-border/60 bg-card/40 px-6 py-24 text-center">
              ...
              <p className="mt-6 font-serif text-2xl">Savat hozircha bo'sh</p>
              <p className="mt-2 max-w-md text-sm text-muted-foreground">
                Kolleksiyamizdan o'zingizga yoqqan tarix bo'lagini tanlang.
              </p>
              <Link
                to="/shop"
            ...
            <div className="mt-12 grid grid-cols-1 gap-10 lg:grid-cols-[1fr_380px]">
              ...
              <div className="space-y-4">
                {items.map(({ product, qty }) => (
                  <div
                    key={product.id}
                    className="group flex flex-col gap-4 rounded-2xl border border-border/60 bg-card/60 p-4 transition-smooth hover:border-primary/40 sm:flex-row sm:items-center"
                  >
                    <Link
                      to="/product/$id"
              ...
              <aside className="h-fit rounded-2xl border border-primary/30 bg-card/80 p-7 shadow-glow lg:sticky lg:top-28">
                ...
                <h3 className="mt-3 font-serif text-2xl">Buyurtma yakuni</h3>

                <dl className="mt-6 space-y-3 text-sm">
                  <Row label="Mahsulotlar" value={`${formatPrice(subtotal)} so'm`} />
                  <Row label="Yetkazib berish" value={`${formatPrice(SHIPPING)} so'm`} />
                </dl>
                ...
                <button
                  onClick={() => toast("Buyurtma qabul qilindi (demo)")}
                ...
                <p className="mt-4 text-center text-[11px] text-muted-foreground">
                  Xavfsiz to'lov · 14 kun qaytarish
                </p>
              </aside>
            </div>
          )}
