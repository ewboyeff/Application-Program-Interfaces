...
export const fundsApi = {
  ...
  async create(data: Partial<Fund> & { category_id?: string; region_id?: string }): Promise<Fund> {
    const res = await adminApiClient.post<DataResponse<any>>('/api/v1/funds', data);
    return mapFund(res.data);
  },

  async update(id: string, data: Partial<Fund> & { category_id?: string; region_id?: string }): Promise<Fund> {
    const res = await adminApiClient.put<DataResponse<any>>(`/api/v1/funds/${id}`, data);
    return mapFund(res.data);
  },

  async delete(id: string): Promise<void> {
    await adminApiClient.delete(`/api/v1/funds/${id}`);
  },

  async verify(id: string): Promise<Fund> {
    const res = await adminApiClient.post<DataResponse<any>>(`/api/v1/funds/${id}/verify`);
    return mapFund(res.data);
  },

  async recalculateIndex(id: string): Promise<Fund> {
    const res = await adminApiClient.post<DataResponse<any>>(`/api/v1/funds/${id}/recalculate-index`);
    return mapFund(res.data);
  },
