          ...
          <div className="flex items-center justify-center gap-1 flex-wrap">
            {[
              ...
              <React.Fragment key={item.labelKey}>
                <div className="bg-white/15 backdrop-blur-md rounded-2xl p-2 text-center w-[72px] border border-white/10">
                  <div className="text-white/70 text-[7px] font-bold uppercase tracking-wide mb-1 truncate">{t(item.labelKey)}</div>
                  <div className="text-white/40 text-[7px] font-medium mb-1">×{item.weight}</div>
                  <div className="text-white font-black text-base">{item.score}</div>
                </div>
                {idx < 2 && <Plus className="w-3 h-3 text-white/40 shrink-0" />}
              </React.Fragment>
            ))}
            <Equal className="w-3 h-3 text-white/40 shrink-0" />
            <div className="bg-white/25 backdrop-blur-md rounded-2xl p-2 text-center w-[72px] border-2 border-white/40">
              <div className="text-white font-black text-xl">{fund.indexes.overall}</div>
            </div>
          </div>
        </div>
